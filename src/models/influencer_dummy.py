...
class InfluencerDummy(InfluencerModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        user_user_edges, user_user_truth = self.get_training_edges(batch, user_embed, user_embed, hnm=True, rp=False, tp=True, batch_index = batch.batch)
        user_influencer_edges, user_influencer_truth = self.get_training_edges(batch, user_embed, influencer_embed, hnm=True, tp=True, rp=False, batch_index = batch.batch)
        influencer_influencer_edges, influencer_influencer_truth = self.get_training_edges(batch, influencer_embed, influencer_embed, hnm=True, rp=False, radius=self.hparams["influencer_margin"], batch_index = batch.batch)
        ...
        self.log_dict({"train_loss": loss, "train_user_user_loss": sublosses["user_user_loss"], "train_user_influencer_loss": sublosses["user_influencer_loss"], "train_influencer_influencer_loss": sublosses["influencer_influencer_loss"]})
    ...
    def shared_evaluation(self, batch, batch_idx, val_radius = None):
        ...
        try:
            user_user_edges, user_user_truth = self.get_training_edges(batch, user_embed, user_embed, hnm=True, knn=500, batch_index=batch.batch, radius=val_radius, self_loop=True)
        except Exception:
        ...
        true_user_influencer_edges = user_influencer_edges[:, user_influencer_truth.nonzero().squeeze()]
        fake_user_influencer_edges = user_influencer_edges[:, (~user_influencer_truth).nonzero().squeeze()]
        user_influencer_distances_true = (user_embed[true_user_influencer_edges[0]] - influencer_embed[true_user_influencer_edges[1]]).norm(dim=-1)
        user_influencer_distances_fake = (user_embed[fake_user_influencer_edges[0]] - influencer_embed[fake_user_influencer_edges[1]]).norm(dim=-1)
        ...
        hist_true_user_influencer_dist = torch.histc(user_influencer_distances_true, bins=10, min=0, max=1)
        hist_fake_user_influencer_dist = torch.histc(user_influencer_distances_fake, bins=10, min=0, max=1)

        true_influencer_influencer_edges = influencer_influencer_edges[:, influencer_influencer_truth.nonzero().squeeze()]
        fake_influencer_influencer_edges = influencer_influencer_edges[:, (~influencer_influencer_truth).nonzero().squeeze()]
        influencer_influencer_distances_true = (influencer_embed[true_influencer_influencer_edges[0]] - influencer_embed[true_influencer_influencer_edges[1]]).norm(dim=-1)
        influencer_influencer_distances_fake = (influencer_embed[fake_influencer_influencer_edges[0]] - influencer_embed[fake_influencer_influencer_edges[1]]).norm(dim=-1)
        avg_true_influencer_influencer_dist = influencer_influencer_distances_true.mean()
        avg_fake_influencer_influencer_dist = influencer_influencer_distances_fake.mean()
        median_true_influencer_influencer_dist = influencer_influencer_distances_true.median()
        median_fake_influencer_influencer_dist = influencer_influencer_distances_fake.median()
        hist_true_influencer_influencer_dist = torch.histc(influencer_influencer_distances_true, bins=10, min=0, max=1)
        hist_fake_influencer_influencer_dist = torch.histc(influencer_influencer_distances_fake, bins=10, min=0, max=1)

        true_user_user_edges = user_user_edges[:, user_user_truth.nonzero().squeeze()]
        fake_user_user_edges = user_user_edges[:, (~user_user_truth).nonzero().squeeze()]
        user_user_distances_true = (user_embed[true_user_user_edges[0]] - user_embed[true_user_user_edges[1]]).norm(dim=-1)
        user_user_distances_fake = (user_embed[fake_user_user_edges[0]] - user_embed[fake_user_user_edges[1]]).norm(dim=-1)
        ...
        hist_true_user_user_dist = torch.histc(user_user_distances_true, bins=10, min=0, max=1)
        hist_fake_user_user_dist = torch.histc(user_user_distances_fake, bins=10, min=0, max=1)
        ...
        return {"cluster_eff": cluster_eff, "cluster_pur": cluster_pur, "represent_pur": represent_pur, "represent_eff": represent_eff, "represent_dup": represent_dup,
                "tracking_eff": tracking_eff, "tracking_fake_rate": 1-tracking_pur, "tracking_dup": tracking_dup}
