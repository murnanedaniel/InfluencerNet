...
class NaiveRegressionModel(ObjectCondensationBase):
    ...
    def get_regression_truth(self, batch):
        ...
        """
        regression_truth = torch.cat([batch[key] for key in self.hparams["regression_targets"]])
    ...
    def training_step(self, batch, batch_idx):
        ...
        _, batch.batched_cluster_id = torch.unique(batch.batched_cluster_id, return_inverse=True)

        # Get the user and influencer embeddings
        input_data = self.get_input_data(batch)

        regression_truth = self.get_regression_truth(batch)        
        regression_predictions = self(input_data, batch = batch.batched_cluster_id, edge_index = batch.get("edge_index"))
        ...
        # Log 
        log_dict = {"train_loss": loss, "train_mae": mae, "train_output_mean": output_mean, "train_output_std": output_std}
    ...
    def shared_evaluation(self, batch, batch_idx):
        ...
        abs_error = torch.nn.functional.l1_loss(regression_predictions, regression_truth)
        rel_error = (torch.abs(regression_predictions - regression_truth) / torch.abs(regression_truth)).mean()
        log_dict = {"val_loss": loss, "lr": lr, "abs_error": abs_error, "rel_error": rel_error, "val_output_mean": output_mean}
        self.log_dict(log_dict, on_epoch=True, on_step=False)
