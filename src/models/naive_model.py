...
class NaiveModel(ObjectCondensationBase):
    ...
    def get_condensation_edges(self, batch, user_embed):
        ...
        candidate_edges = build_edges(user_embed, user_embed, r_max=self.hparams["radius"], k_max=100, batch_index = batch.batch, self_loop=True)
    ...
    def shared_evaluation(self, batch, batch_idx):
        ...
        try:
            user_user_edges, user_user_truth = self.get_training_edges(batch, user_embed, user_embed, hnm=True, knn=500, self_loop=True, batch_index=batch.batch)
            loss = self.get_user_user_loss(user_user_edges, user_user_truth, user_embed)
        except Exception:
