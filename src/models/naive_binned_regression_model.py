...
class NaiveBinnedRegressionModel(ObjectCondensationBase):
    def __init__(self, hparams):
        ...
        self.metrics = MetricCollection({
            'Accuracy': Accuracy(task="multiclass", num_classes=hparams["num_bins"], average='macro'),
            'Precision': Precision(task="multiclass", num_classes=hparams["num_bins"], average='macro'),
            'Recall': Recall(task="multiclass", num_classes=hparams["num_bins"], average='macro'),
        })
    ...
    def get_classification_truth(self, regression_truth, batch):
        classification_truth = convert_targets_to_classes(regression_truth, batch, self.hparams["num_bins"], self.hparams["bin_limits"])
    ...
    def training_step(self, batch, batch_idx):
        ...
        classification_predictions = self(input_data, batch = batch.batched_cluster_id, edge_index = batch.get("edge_index"))
