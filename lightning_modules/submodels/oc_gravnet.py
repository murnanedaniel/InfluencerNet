...
from ..oc_base import OCBase

class OCGravnet(OCBase):
    ...
    def forward(self, x, batch=None):
        ...
        for i, grav_conv in enumerate(self.grav_convs):
            ...
            hidden_features, spatial_edges, _, grav_fact = checkpoint(grav_conv, hidden_features, self.current_epoch, batch)
        ...
        user_out = self.user_network(hidden_features)
        charge_out = torch.sigmoid(self.charge(x))
        ...
        return user_out, charge_out.squeeze()
...
class GravConv(nn.Module):
    ...
    def get_neighbors(self, spatial_features, batch):
        
        edge_index = torch.empty([2, 0], dtype=torch.int64, device=spatial_features.device)
 
        if self.use_radius:
            radius_edges = radius_graph(spatial_features, r=self.r, max_num_neighbors=self.hparams["max_knn"], batch=batch, loop=self.hparams["self_loop"])
            edge_index = torch.cat([edge_index, radius_edges], dim=1)
        
        if self.use_knn and self.knn > 0:
            k_edges = knn_graph(spatial_features, k=self.knn, batch=batch, loop=True)
            edge_index = torch.cat([edge_index, k_edges], dim=1)

        if self.use_rand_k and self.rand_k > 0:
            random_edges = knn_graph(torch.rand(spatial_features.shape[0], 2, device=spatial_features.device), batch=batch, k=self.rand_k, loop=True) 
    ...
    def grav_pooling(self, spatial_features, hidden_features, batch=None):
        edge_index = self.get_neighbors(spatial_features, batch)
        start, end = edge_index
    ...
    def forward(self, hidden_features, current_epoch, batch=None):
        ...
        aggregated_hidden, edge_index, grav_fact = self.grav_pooling(spatial_features, hidden_features, batch)
